...
async function updatePersonalSubmissionHistory() {
    ...
    try {
        ...
        submissionHistory.innerHTML = '<p class="no-submissions">Error loading submission history.</p>';
    }
}
        ...
        if (response.ok) {
            ...
            if (result.status === 'completed' || result.status === 'failed') {
                ...
                if (progressElement && textElement) {
                    ...
                    setTimeout(async () => {
                        // Refresh the table to show final results and update persistence
                        await updatePersonalPerformanceTable();
                        await updatePersonalSubmissionHistory();
                        await updatePersonalStats();
                        await updateLeaderboard();
